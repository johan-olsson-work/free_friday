...
static char* vapix_post(CURL* handle, const char* credentials, const char* endpoint,
                        const char* request) {
    ...
    if (res != CURLE_OK) panic("curl_easy_perform error %d: '%s'", res, curl_easy_strerror(res));

    long response_code;
    curl_easy_getinfo(handle, CURLINFO_RESPONSE_CODE, &response_code);
    if (response_code != 200)
        panic("Got response code %ld from request to %s with response '%s'", response_code, request,
              response->str);
...
static void upload_and_install_acap(CURL* handle, const char* credentials) {
    const char* eap_file_path = "/usr/local/packages/vapix_example/localdata/hello_world.eap";
    FILE* eap_file            = fopen(eap_file_path, "rb");
    if (!eap_file) {
        panic("Failed to open EAP file: %s", eap_file_path);
    }
    ...
    if (!mime) {
        panic("Failed to create curl_mime handle");
    }

    field = curl_mime_addpart(mime);
    curl_mime_name(field, "packfil");
    curl_mime_filedata(field, eap_file_path);

    curl_easy_setopt(handle, CURLOPT_URL,
                     "http://127.0.0.12/axis-cgi/admin/applications/upload.cgi");
    curl_easy_setopt(handle, CURLOPT_USERPWD, credentials);
    curl_easy_setopt(handle, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
    ...
    curl_easy_getinfo(handle, CURLINFO_RESPONSE_CODE, &response_code);
    if (response_code != 200) {
        panic("Got response code %ld from upload request", response_code);
    }
...
static json_t* vapix_post_json(CURL* handle, const char* credentials, const char* endpoint,
                               const char* request) {
    ...
    if (!json_response) panic("Invalid JSON response: %s", parse_error.text);
...
int main(void) {
    ...
    upload_and_install_acap(handle, credentials);
